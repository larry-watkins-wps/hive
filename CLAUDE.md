...
- **Bootstrap mode (active 2026-04-29 onward, per Larry).** While Hive is developing — empty `handlers/` directories, missing cognitive wiring, no audio/visual hardware bridges — hand-editing `regions/<name>/` files (subscriptions, handlers, prompts, configs) is permitted to bootstrap and iterate. Region sovereignty (Principle III) and sleep-only self-modification (Principle IX) become operative constraints once each region has functioning cognition; until then they would block the work needed to get there. When you make a region-side edit, restart that region's container so the runtime picks it up. Once a region is reliably deliberating + sleeping autonomously, transition its changes back to the cosign-gated `hive/system/codechange/proposed` path.
- **Cross-region edits are still scoped.** Even in bootstrap, if you are working on `regions/<A>/` for one task, don't drift into editing `regions/<B>/` in the same change unless the task explicitly spans both. Make the cross-region nature of the change visible in the commit message.
- **`region_template/` (DNA) remains a separate boundary.** Edits there propagate to every region simultaneously and stay on the cosign-gated path even during bootstrap.
