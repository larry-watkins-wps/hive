...
4. **Experience-grounded** — regions read observable state (modulator values, STM depth, consolidated LTM, appendix history) to shape behavior. Developmental stage and age are emergent from accumulated experience, not declared labels; no region reads or branches on a stage field.
...
| File | Layer | Role | LLM |
|---|---|---|---|
| `medial_prefrontal_cortex.md` | Self | identity, values, personality (DMN hub) | Opus 4.6 |
...
5. **How you reason** — decision heuristics, modulator sensitivity, experience-grounded adaptation
