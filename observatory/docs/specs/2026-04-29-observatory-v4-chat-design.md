...
- **`source_region`** = `"observatory_sensory"` — explicit non-region origin. The translator is not a brain region and shouldn't pretend to be one. The wrapper field is constrained by `shared/message_envelope.py`'s schema regex `^[a-z][a-z0-9_]{2,30}$` (region-name shaped), so the underscore form is required even though no region of that name exists; consumers that want to filter by "is this a real cortex output" can check the `observatory_` prefix. (Earlier draft used `observatory.sensory`; the dot violates the schema and assoc_cortex's poison-message handler rejected the envelope. Fixed in the post-ship `observatory_sensory` patch — see [HANDOFF.md](../../HANDOFF.md).)
...
- Constructs the envelope via `Envelope.new(source_region="observatory_sensory", topic="hive/external/perception", content_type="application/json", data={...})` (the factory generates `id` and `timestamp`).
