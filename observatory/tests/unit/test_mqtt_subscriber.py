...
def test_subscription_map_production_schema(tmp_path) -> None:
    """Real Hive region files use `subscriptions: [{topic, qos, description}]`
    rather than a flat `topics: [strings]` list. The loader must honor that
    schema or the observatory produces an empty subscription map, which
    cascades to empty `destinations` and a permanently empty adjacency
    matrix (no edge threads, no sparks).
    """
    from observatory.mqtt_subscriber import load_subscription_map  # noqa: PLC0415

    (tmp_path / "regions").mkdir()
    r = tmp_path / "regions" / "amygdala"
    r.mkdir()
    (r / "subscriptions.yaml").write_text(
        "schema_version: 1\n"
        "subscriptions:\n"
        ...
        "    qos: 0\n"
        "  - topic: hive/sensory/visual/processed\n"
        "    qos: 0\n"
        "    description: Threat-relevant visual frames\n",
        encoding="utf-8",
    )
    m = load_subscription_map(tmp_path)
    assert m == {
...
def test_subscription_map_skips_records_missing_topic(tmp_path) -> None:
    """A record with no `topic:` key (e.g. dangling qos) must be dropped
    silently, not crash the loader or pollute the map with None."""
    from observatory.mqtt_subscriber import load_subscription_map  # noqa: PLC0415
    ...
    m = load_subscription_map(tmp_path)
    assert m == {"thalamus": ["hive/ok", "hive/also_ok"]}


def test_subscription_map_unions_both_schemas(tmp_path) -> None:
    """If a region file contains BOTH `subscriptions:` and `topics:` we
    union them, preserving first-seen order and deduping. This covers
    migration windows where a region hand-adds a topic to `topics:` on
    top of an autogenerated `subscriptions:` block.
    """
    from observatory.mqtt_subscriber import load_subscription_map  # noqa: PLC0415

    (tmp_path / "regions").mkdir()
    r = tmp_path / "regions" / "broca_area"
    r.mkdir()
    (r / "subscriptions.yaml").write_text(
        "subscriptions:\n"
        "  - topic: hive/a\n"
        "  - topic: hive/b\n"
        "topics:\n"
        "  - hive/b       # duplicate, must not appear twice\n"
        "  - hive/c\n",
        encoding="utf-8",
    )
    m = load_subscription_map(tmp_path)
    assert m == {"broca_area": ["hive/a", "hive/b", "hive/c"]}
