"""Respond handler for prefrontal_cortex — LLM-driven deliberation.

PFC's primary deliberation input is the cross-modal synthesis that
association_cortex publishes on ``hive/cognitive/association_cortex/
integration``. PFC also deliberates on motor outcome topics (prediction
error / feedback): ``hive/motor/{complete,failed,partial,speech/complete}``.

It calls the LLM with the region's prompt.md as the system message and
parses the response according to the tag schema documented in prompt.md
...
PFC's expected output topics are:
    - ``hive/cognitive/prefrontal_cortex/plan`` (always emitted)
    - ``hive/motor/intent``                      (when a non-verbal action is warranted)
    - ``hive/motor/speech/intent``               (when a verbal response is warranted)
...
``_parse_response``) are a verbatim copy of the association_cortex
respond.py template (commits 878ec37 + 19e035a). Deduplication into a
shared module is deferred until the handler cohort lands.
"""
...
try:
    ...
    _SYSTEM_PROMPT = "You are the Prefrontal Cortex of Hive."
...
# tests/unit/regions/prefrontal_cortex/test_handlers.py
...
def _build_user_message(envelope, recent_events: list[dict]) -> str:
    ...
    if isinstance(data, dict):
        ...
        # Integration synthesis envelopes from assoc_cortex carry these.
        modalities_integrated = data.get("modalities_integrated")
        confidence = data.get("confidence")
    else:
        text = str(data) if data is not None else ""
        speaker = "unknown"
        modality = "unknown"
        modalities_integrated = None
        confidence = None

    extra = ""
    if modalities_integrated is not None:
        extra += f"modalities_integrated: {modalities_integrated}\n"
    if confidence is not None:
        extra += f"upstream_confidence: {confidence}\n"
    ...
    return (
        ...
        f"Deliberate using the tag schema described in your prompt. "
        f"Always emit at least one <publish topic=\"hive/cognitive/prefrontal_cortex/plan\"> "
        f"block summarizing your conclusion."
    )
...
async def handle(envelope, ctx):
    ...
    req = CompletionRequest(
        ...
        max_tokens=2048,
        temperature=0.7,
        purpose="prefrontal_cortex.respond",
    )
    ...
    try:
        ...
        if parse_error is not None:
            ctx.log.warning(
                "prefrontal_cortex.respond.parse_failure",
                topic=envelope.topic,
                reason=parse_error,
        ...
        for topic, body in publishes:
            ...
            ctx.log.info(
                "prefrontal_cortex.respond.published",
                topic=topic,
